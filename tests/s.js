...
var S = function S(fn, value) {
    ...
    if (Owner === null)
        ...
        var _a = makeComputationNode(fn, value, false, false),
            node = _a.node,
            _value = _a.value;
    // console.log("S computationNode:", _a);
    if (node === null) {
...
S.data = function data(value) {
    ...
    console.log("--> setup DATA NODE:", node);
    return function data(value) {
        if (arguments.length === 0) {
            console.log("--> setup DATA:", node.current);
            return node.current();
        } else {
